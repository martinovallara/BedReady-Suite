...
export default async function useCaseStartDateInput() {
  ...
  const eventRepository = await EventsRepository.getInstance();
  useCaseBaddingSetStateReport(eventRepository).setStartDateReport(parseJSDate(startDate, "dd/LL/yy"));
}
